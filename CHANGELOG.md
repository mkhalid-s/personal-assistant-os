...
- Promote `mypy --strict` on `execution.py` from advisory to **fatal** in CI, closing the safety-critical mypy-strict tier. `execution.py` is the approve-and-execute core: it owns the hard destructive guard (`autonomy.BLOCKED`), the approval integrity check (payload hash pinning + TTL enforcement), every `execution_receipt` write, connector mutation dispatch, and the follow-up-inbox creation on failed/blocked receipts. A type regression here can silently mis-classify or corrupt an approval before it's audited. Fifteen annotation-debt findings were fixed (`no-untyped-def` on 9 helpers gained explicit `conn: sqlite3.Connection`/`row: Any` annotations, `dict` return types were parameterized as `dict[str, Any]`, and two `resp.read().decode(...)` returns got typed narrowing), plus one real defensive-correctness fix: `_verification_receipt_context` now `isinstance`-narrows `payload["verification_commands"]` to `(list, tuple)` before iterating — a scalar (e.g. a single string) would previously have iterated as characters and produced garbage receipt commands. One `cur.lastrowid` narrowing (`assert cur.lastrowid is not None`) was added to satisfy the `Optional[int]` return of the sqlite3 stubs. The fatal `mypy --strict` step in CI now covers the full safety-critical set: `command_registry.py` + `privacy.py` + `db.py` + `zero_executor.py` + `execution.py`. The advisory mypy step is retired; whole-tree `ruff` and `pip-audit` remain advisory pending their own promotion slices.
- Promote `mypy --strict` on `zero_executor.py` from advisory to **fatal** in CI. `zero_executor.py` is the external coding-executor bridge — it subprocesses `zero exec`, parses schema-versioned stream JSON (`SCHEMA_VERSION = 2`), and persists redacted `agent_runs` rows. A type regression here can silently drop tool events, mis-attribute exit codes, or corrupt the recorded payload. Two pre-existing findings were fixed: (1) `run_end.exitCode` is now `isinstance`-narrowed to `(int, str, float)` before `int(...)` so mypy sees a safe conversion and unexpected shapes get recorded as a `protocol_error` line instead of raising, and (2) `record_zero_agent_run` gained an explicit `conn: sqlite3.Connection` annotation. The fatal `mypy --strict` step in CI now covers `command_registry.py` + `privacy.py` + `db.py` + `zero_executor.py`; only `execution.py` remains in the advisory tier.
- Promote `mypy --strict` on `db.py` from advisory to **fatal** in CI. `db.py` is MYOS's single mutation and schema boundary (~1.7 kloc covering connection management, migration application, event append, integrity binding, and payload hashing); a strict regression here can silently corrupt local state before any privacy or approval filter runs. `db.py` was already clean under `mypy --strict` — no code changes were required, just promotion in `.github/workflows/ci.yml`. The fatal step now covers `command_registry.py` + `privacy.py` + `db.py`; the remaining advisory tier is `execution.py` + `zero_executor.py`, both scheduled for the next slices in this sequence.
