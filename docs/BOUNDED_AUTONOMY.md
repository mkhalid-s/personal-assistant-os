...
- Enforce static type + lint safety on the CLI contract and execution core: CI runs whole-tree `ruff check` and `ruff format --check` fatally, plus `mypy --strict` on `command_registry`, `privacy`, `db`, `approval_context`, `autonomy`, `inbox`, `agentcore`, `zero_executor`, and `execution`. `pip-audit` remains advisory until the project defines an upstream-vulnerability exception policy.
...
- Approval integrity is now visible at review time, not just at execution time. `myos approve --list` (both text and JSON) surfaces a compact `integrity` block per queued action with `state` ∈ {`not_yet_approved`, `fresh`, `nearing_expiry`, `expired`, `tampered`, `invalid`}, plus the pinned `payload_hash`, `approved_at`, and remaining TTL. The `nearing_expiry` band covers the last 10% of the TTL window so operators can proactively re-approve before the execution refusal path kicks in. This closes the gap between the enforcement (added earlier via `payload_hash` + `approved_at` schema binding + `verify_approval_integrity`) and the operator experience — a supervisor now sees "3 approvals nearing expiry, 1 tampered" in the same queue view they already scan.
