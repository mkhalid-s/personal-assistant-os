...
| --- | --- | --- | --- | --- |
| **ECC** | Governance/skills/memory layer installed *into* coding harnesses (Claude Code, Codex, and similar IDEs). 68 agents, 286 skills, 94 commands, hooks, "instincts" (confidence-scored learned patterns), Memory Vault, AgentShield scanner (102 rules, CI gate), Plan Canvas approval UI. ~251k stars self-reported. | TS/Shell/Python | Adapters per harness; marketplace plugins; markdown skills | Explicit hook-consent at install; secret-blocking editor hooks; AgentShield exit-2 CI gate; "memory is unreviewed context, not executable policy" |
