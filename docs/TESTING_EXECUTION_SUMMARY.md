...
## Agent CLI (`cursor` backend) integration check
...
- **`available()`**: `True` — logged-in Agent CLI (`agent status` healthy)
...
The Agent CLI also drove the CI recovery on `main` (whitespace + ruff format), including Public Hygiene / Static Checks going green after retry: https://github.com/mkhalid-s/personal-assistant-os/actions/runs/35501951073
