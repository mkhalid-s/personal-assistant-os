...
Integer **millicents** throughout (100_000 millicents = $1) — no float
drift; divide by 100_000 at display time. `self_reported_cost_*` exists so
...
Status: **Slices 1–3 shipped** on `feat/llm-usage-ledger` (ledger + default-backend capture, governance + surfaces, subprocess/estimation/router events + `myos today` block). Price rates are placeholders pending a refresh from the Anthropic pricing page.
